...
def gawang():
	global x_gawang
	global y_gawang
	global radius_gawang
	Bool,video = cap.read()
	video = cv2.flip(video,0)
	datagawang = np.array([0,0,0])
	hsv = cv2.cvtColor(video,cv2.COLOR_BGR2HSV)
	...
	if len(cnts) > 0:
		c = max(cnts, key=cv2.contourArea)
		((x_gawang,y_gawang),radius_gawang) = cv2.minEnclosingCircle(c)
		datagawang = np.array([int(x_gawang), int(y_gawang), int(radius_gawang)])
		#if radius > 10:
			#cv2.circle(video, (int(x), int(y)), int radius(), colors,2)
	else:
		datagawang = np.array([0,0,0])
	print'data gawang = (%d,%d,%d)'%(x_gawang,y_gawang,radius_gawang)	
	return datagawang
